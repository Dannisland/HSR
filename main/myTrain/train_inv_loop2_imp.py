...
from models.imp_subnet_DeepMIH import *
from models import get_model_inv
from models.model_DeepMIH import *
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if cfg.sync_bn:
        ...
        imp_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(imp_model)
    ...
    if cfg.distributed:
        ...
        imp_model = torch.nn.parallel.DistributedDataParallel(imp_model.cuda(gpu), device_ids=[gpu])
    ...
    if cfg.use_sgd:
        optimizer = torch.optim.SGD(
            itertools.chain(inv_model.parameters(), inv_crm.parameters(), inv_model2.parameters(),
                            inv_crm2.parameters(), imp_model.parameters()), lr=cfg.base_lr,
            momentum=cfg.momentum,
            weight_decay=cfg.weight_decay)
    else:
        optimizer = torch.optim.Adam(
            itertools.chain(inv_model.parameters(), inv_crm.parameters(), inv_model2.parameters(),
                            inv_crm2.parameters(), imp_model.parameters()),
            lr=1e-05, eps=cfg.eps, weight_decay=cfg.weight_decay, betas=(0.5, 0.999))
    ...
    if cfg.weight:
        if os.path.isfile(cfg.weight):
            ...
            load_state_dict(imp_model, checkpoint['imp_model'], strict=False)
    ...
    if cfg.resume:
        if os.path.isfile(cfg.resume):
            ...
            load_state_dict(imp_model, checkpoint['imp_model'])
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs):
        ...
        loss_train, hr_loss, _ = train(train_loader, inv_model, inv_crm, inv_model2, inv_crm2, imp_model, loss, optimizer, epoch,
                                       cfg)
        epoch_log = epoch + 1
        # # Adaptive LR
        if cfg.StepLR:
        ...
        if cfg.evaluate and (epoch_log % cfg.eval_freq == 0):
            loss_val, hr_loss, _, PSNR, SSIM = \
                validate(val_loader, inv_model, inv_crm, inv_model2, inv_crm2, imp_model, loss, epoch, cfg)
            if main_process(cfg):
        ...
        if (epoch_log % cfg.save_freq == 0) and main_process(cfg):
            save_checkpoint_loop2_imp(inv_model,
                                  inv_crm,
                                  inv_model2,
                                  inv_crm2,
                                  imp_model,
                                  ...
                                  other_state={
                                      ...
                                      'imp_model': imp_model.state_dict(),
...
def train(train_loader, inv_model, inv_crm, inv_model2, inv_crm2, imp_model, loss_fn, optimizer, epoch, cfg):
    ...
    for i, batch in enumerate(train_loader):
        ...
        #   forward-2:  #
        #################
        imp_map = imp_model(lr, sec, output_stego)
        # impmap_loss = loss_fn[0](imp_map, lr - output_stego)
        imp_map_dwt = dwt(imp_map)  # channels = 12   imp模块
        ...
        input_img_2 = torch.cat((cover_input_2, imp_map_dwt), 1).cuda(cfg.gpu, non_blocking=True)
        input_img_2 = torch.cat((input_img_2, secret_input_2), 1).cuda(cfg.gpu, non_blocking=True)
        ...
        recovered_fre_2 = output_image_2.narrow(1, output_image_2.shape[1] - 12, 12)
...
def validate(val_loader, inv_model, inv_crm, inv_model2, inv_crm2, imp_model, loss_fn, epoch, cfg):
    loss_meter = AverageMeter()
    loss_hr_meter = AverageMeter()
    loss_hr_meter2 = AverageMeter()
    loss_sec_meter = AverageMeter()
    ...
    with torch.no_grad():
        for step, batch in enumerate(val_loader):
            ...
            loss = loss_hr + loss_sec + loss_fre_cover + loss_fre_secret + loss_fre_inv + \
                   loss_hr_2 + loss_sec_2 + loss_fre_cover_2 + loss_fre_secret_2 + \
                   loss_hr_dist + loss_sec_dist + loss_fre_secret_dist
