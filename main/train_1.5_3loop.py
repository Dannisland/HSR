...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs):
        ...
        loss_train, hr_loss, _ = train(train_loader, model, revealNet, revealNet_2, revealNet_3, loss, optimizer, epoch,
                                       cfg)
        epoch_log = epoch + 1
        # # Adaptive LR
        if cfg.StepLR:
            ...
            if main_process(cfg):
                ...
                for m, s in zip([loss_val, hr_loss, *PSNR, *SSIM],
                                ["val/loss", "val/loss_hr",
                                 "val/PSNR_lr", "val/PSNR_hr", "val/SSIM_lr", "val/SSIM_hr",
                                 "val/PSNR_lr_2", "val/PSNR_hr_2", "val/SSIM_lr_2", "val/SSIM_hr_2",
                                 "val/PSNR_lr_3", "val/PSNR_hr_3", "val/SSIM_lr_3", "val/SSIM_hr_3"
                                 ]):
                    writer.add_scalar(s, m, epoch_log)
...
def train(train_loader, model, revealNet, revealNet_2, revealNet_3, loss_fn, optimizer, epoch, cfg):
    ...
    for i, batch in enumerate(train_loader):
        ...
        loss = loss_hr + loss_sec + loss_hr_2 + loss_hr_3 + loss_sec_2 + loss_sec_3
        ...
        if (i + 1) % cfg.print_freq == 0 and main_process(cfg):
            ...
            for m, s in zip([loss_meter, loss_hr_meter, loss_sec_meter, loss_hr_meter2,
                             loss_sec_meter2, loss_hr_meter3, loss_sec_meter3],
        ...
        for step, batch in enumerate(val_loader):
            ...
            for m, x in zip([loss_meter, loss_hr_meter, loss_sec_meter, *psnr_meter, *ssim_meter],
                            [loss, loss_hr, loss_sec,
                             psnr_lr, psnr_hr, psnr_lr_2, psnr_hr_2, psnr_lr_3, psnr_hr_3,
