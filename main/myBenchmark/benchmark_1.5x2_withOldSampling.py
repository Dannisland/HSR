...
def main():
    ...
    for data_name in data_names:
        ...
        for scale in test_scales:
            ...
            test(model, revealNet, revealNet_2, test_loader, scale=float(scale), save=cfg.save, data_name=cfg.data_name)


def test(model, revealNet, revealNet_2, test_data_loader, scale=4, save=False, data_name=None):
    ...
    with torch.no_grad():
        ...
        with open(filepath, 'w') as f:
            for i, batch in enumerate(tqdm(test_data_loader)):
                ...
                BicResHr = nn.functional.interpolate(lr_1_4, [lr_1_2.shape[2], lr_1_2.shape[3]], mode="bicubic")
                BicResHr2 = nn.functional.interpolate(restored_hr, [hr.shape[2], hr.shape[3]], mode="bicubic")
                ...
                rev_dist = torch.clamp(rev_dist, 0, 1)
                ...
                BicResHr = torch.clamp(BicResHr, 0, 1)
                BicResHr = util.tensor2img(BicResHr)
                BicResHr2 = torch.clamp(BicResHr2, 0, 1)
                BicResHr2 = util.tensor2img(BicResHr2)
                ...
                # psnr_meter_rev_dist.update(util.calculate_psnr(BicResHr * 255, lr_1_2 * 255))
                # psnr_meter_rev_redist.update(util.calculate_psnr(BicResHr2 * 255, hr * 255))
                ...
                # ssim_meter_rev_1_4.update(util.calculate_ssim(restored_hr * 255, lr_1_2 * 255))
                # ssim_meter_rev_1_2.update(util.calculate_ssim(restored_hr2 * 255, hr * 255))
